...
## Домашнее задание № 25 (CI/CD в Kubernetes)

Что сделано:
1. Установлен Helm (клиентская и серверная часть — Tiller).
2. Описаны chart'ы для компонентов приложения с использованием шаблонов и helper'ов.
3. С помощью механизма управления зависимостями создан единый chart приложения reddit, который объединяет компоненты приложения.
4. Командой `helm install --name <release-name> <chart-path>` приложение развёрнуто в кластере k8s.
5. Установлен Gitlab:
    - в кластере GKE добавлен новый пул;
    - скачан Chart Gitlab'a;
    - файлы Chart'a модифицированы под наши нужды;
    - при помощи Helm Gitlab установлен в кластере;
    - в файл hosts добавлен внешний IP-адрес ingress-контроллера nginx для доступа к создаваемым окружениям.
6. Gitlab настроен (создана группа, добавлены учётные данные от Docker Hub), подготовлены проекты reddit-deploy, ui, comment и post.
7. Локально размещены файлы для проектов, для каждого проекта создан репозиторий.
8. Для каждого проекта создано описание пайплайна в виде файла .gitlab-ci.yml.
9. Обновлён конфиг ингресса для сервиса UI для работы через nginx.
10. Отредактированы файлы .gitlab-ci.yml для автоматического создания окружения при коммитах, добавлен шаг по уничтожению созданного окружения, запускаемый вручную.
11. Создан файл reddit-deploy/.gitlab-ci.yml для разворачивания тестового и рабочего окружений и деплоя в них приложения reddit.

## Домашнее задание № 26 (Kubernetes. Мониторинг и логирование)

Что сделано:
1. Предварительно настроен кластер k8s (отключены Stackdriver Logging и Stackdriver Monitoring).
2. Установлен ingress-контроллер nginx, его IP-адрес добавлен в **hosts**.
3. Исследована работа Prometheus:
    - загружен и сохранён локально chart Prometheus'а , внутри директории чарта создан файл **custom_values.yaml** с изменёнными настройками. При помощи Helm запущен Prometheus;
    - в файле **custom_values.yml** включены сервисы `kube-state-metrics` и `node-exporter` для сбора дополнительной информации;
    - запущено приложение из helm-чарта **reddit**;
    - в файл **custom_values.yml** добавлены и настроены (добавлены лейблы k8s и Prometheus) job'ы для снятия метрик отдельно для каждого из компонентов приложения, а также для разных окружений.
4. Исследована работа Grafana:
    - при помощи Helm установлена Grafana, произведены нужные настройки;
    - добавлен дашборд для отслеживания состояния ресурсов k8s, а также собственные дашборды, созданные ранее;
    - добавлена переменная `namespace` и настроен механизм отображения метрик в зависимости от выбранного пространства имён.
5. Исследована работа EFK-стека:
    - самой мощной ноде в кластере добавлена метка `elastichost=true`;
    - при помощи YAML-манифестов и команды `kubectl apply` в кластере установлены Elasticsearch и Fluentd;
    - при помощи Helm установлена Kibana, создан шаблон индекса.
